...
func (gen CodeGenerator) imports() ast.Decl {
	...
	for importName, alias := range gen.packageAlias {
		...
		v := strings.Replace(importName, "\\", "/", -1)
...
func printImportSpecs(specs map[string]*ast.ImportSpec) {
	for i := range specs {
		log.Println(specs[i].Name)
		log.Println(specs[i].Path.Kind.String())
		log.Println(specs[i].Path.Value)
	}
}

func printPackageAlias(alias map[string]string) {
	for k, v := range alias {
		log.Printf("%s: %s", k, v)
	}
}
...
func (gen CodeGenerator) fixup() {
	printPackageAlias(gen.packageAlias)
	for _, m := range gen.Model.Methods {
		typ := m.Field.Type.(*ast.FuncType)
		log.Println(m.Field.Names[0].Name)
		printImportSpecs(m.Imports)
		log.Printf("Before:\n\n%+v\n", typ)
